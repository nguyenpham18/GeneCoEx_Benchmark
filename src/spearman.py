""" Spearman similarity-metric setup for gene co-expression benchmarking.

Input: cleaned gene expression matrix (genes as rows, samples as columns by default).
Output: Spearman gene-gene correlation matrix.
"""
...
def compute_spearman_similarity(expr: pd.DataFrame, min_periods: int = 3) -> pd.DataFrame:
    logger.info(
        "Computing Spearman correlation for %d genes (min_periods=%d).",
        expr.shape[0],
        min_periods,
    )

    # Rank each gene's expression across samples (axis=1 ranks across columns/samples)
    # This converts the expression matrix into rank matrix
    logger.info("Ranking expression values across samples...")
    ranked = expr.rank(axis=1, method="average", na_option="keep")
    logger.info("Ranking done. Computing Pearson correlation on ranked data...")

    # Spearman = Pearson correlation of ranks
    corr = ranked.T.corr(method="pearson", min_periods=min_periods)
...
def summarize_similarity(corr: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "std_correlation": float(np.std(valid)) if len(valid) else float("nan"),
...
def save_similarity_matrix(corr: pd.DataFrame, output_path: str | Path) -> None:
    ...
    logger.info("Saved Spearman similarity matrix to: %s", output_path)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute Spearman gene-gene similarity matrix."
    )
    parser.add_argument("--input",  required=True, help="Path to cleaned expression matrix.")
    parser.add_argument("--output", required=True, help="Path to save the Spearman similarity matrix.")
    ...
    corr = compute_spearman_similarity(expr, min_periods=args.min_periods)
    summary = summarize_similarity(corr)

    logger.info("Spearman similarity matrix computed successfully.")
